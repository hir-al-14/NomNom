begin;
alter table public.restaurants add column if not exists photo text not null default '';
alter table public.dishes add column if not exists category text not null default 'Mains';
alter table public.dishes drop constraint if exists dishes_category_check;
alter table public.dishes add constraint dishes_category_check check
  (category in ('Bowls','Salads','Sandwiches','Mains','Soups','Sides','Desserts','Drinks'));
...
alter table public.restrictions add constraint restrictions_tag_check check (
  tag in ('gluten','dairy','peanuts','tree-nuts','eggs','soy','sesame','fish','shellfish','high-fiber','high-sodium','hard-texture')
  or (left(tag,7)='custom:' and char_length(tag)<=87 and char_length(btrim(substr(tag,8)))>0)
);
...
begin
  ...
  if expected_version = 0 then
    insert into public.restaurants(id, owner_id, name, cuisine, address, hours, photo)
    values(rid, auth.uid(), payload->'profile'->>'name', coalesce(payload->'profile'->>'cuisine',''),
      coalesce(payload->'profile'->>'address',''), coalesce(payload->'profile'->>'hours',''), coalesce(payload->'profile'->>'photo',''));
    next_version := 1;
  else
    update public.restaurants set name = payload->'profile'->>'name', cuisine = payload->'profile'->>'cuisine',
      address = payload->'profile'->>'address', hours = payload->'profile'->>'hours', photo = coalesce(payload->'profile'->>'photo',''), version = version + 1
  ...
  for item in select value from jsonb_array_elements(payload->'dishes') loop
    ...
    insert into public.dishes(id, restaurant_id, name, description, price_cents, ingredients, flags, flag_notes, nutrition, photo, complete, category)
    values(item->>'id', rid, item->>'name', coalesce(item->>'description',''), (item->>'priceCents')::integer,
      item->'portions', array(select jsonb_array_elements_text(item->'flags')), item->'flagNotes', item->'nutrition', item->>'photo', (item->>'complete')::boolean, coalesce(item->>'category','Mains'))
    on conflict(id) do update set name = excluded.name, description = excluded.description, price_cents = excluded.price_cents,
      ...
      nutrition = excluded.nutrition, photo = excluded.photo, complete = excluded.complete, category = excluded.category, available = true
