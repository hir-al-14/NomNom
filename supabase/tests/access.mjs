...
const expanded = fs.readFileSync(root + '/supabase/expand-catalog.sql', 'utf8');
await db.exec(expanded); await db.exec(expanded);
if ((await db.query('select * from public.restaurants')).rows.length !== 7) throw Error('Expanded restaurants missing');
if ((await db.query('select * from public.dishes')).rows.length !== 26) throw Error('Expanded dishes missing or duplicated');
if ((await db.query("select price_cents from public.dishes where id='window-latte'")).rows[0].price_cents !== 725) throw Error('Expanded catalog changed price');
await db.exec("update public.restaurants set version=2 where id='window'; update public.dishes set description='Owner recipe' where id='window-latte'");
await db.exec(expanded);
if ((await db.query("select description from public.dishes where id='window-latte'")).rows[0].description !== 'Owner recipe') throw Error('Owner recipe overwritten');
await asUser(customer);
await db.query("select public.save_restrictions($1::jsonb)", [JSON.stringify([{tag:'sesame',severity:'high'},{tag:'soy',severity:'medium'}])]);
const placed = (await db.query("select id from public.orders where restaurant_id='window'")).rows[0].id;
await asUser(owner);
const conversation = (await db.query('select public.open_order_chat($1)', [placed])).rows[0].open_order_chat;
await db.query('insert into public.chat_messages(thread_id,sender_id,body) values($1,auth.uid(),$2)',[conversation,'Your order is ready']);
await asUser(customer);
if (!(await db.query('select * from public.chat_messages where thread_id=$1',[conversation])).rows.length) throw Error('Customer cannot read owner chat');
await asUser(stranger); await denied('select public.open_order_chat($1)',[placed]);
console.log('PASS: expanded catalog, editable categories, new restrictions and order conversations');
await db.close();
