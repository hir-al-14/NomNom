...
export function emptyDish(id: string): MenuDish {
  return { id, restaurantId: 'window', name: '', description: '', category: 'Mains', priceCents: 0,
...
export function restaurantSeed(): RestaurantData {
  const make = (id: string, name: string, photo: MenuDish['photo'], ingredients: string[], flags: MenuDish['flags']) => ({
    ...emptyDish(id), name, photo, ingredients, flags, category: photo === 'cupcake' ? 'Desserts' : photo === 'latte' ? 'Drinks' : 'Sandwiches',
  ...
  return {
    profile: { id: 'window', name: 'Window Coffee Bar', cuisine: 'Cafe',
